...
app.post("/process_invoice_page", function (request, response, next) {
  ...
  if (typeof POST['purchase_submit_button'] !== 'undefined') {
    var valid_purchase = true; // variable associated with true
    var qty_num_puppies = false
    for (i = 0; i < products.length; i++) {
      num_puppies = POST[`quantity${i}`];
      //if quantity of puppies is not negative 
      qty_num_puppies = qty_num_puppies || num_puppies > 0;
      //if is positive number & valid integer 
      valid_purchase = valid_purchase && isNonNegInt(num_puppies);
    }

    //if both are valid display invoice
    if (valid_purchase && qty_num_puppies) {

      console.log(Date.now() + ': Purchase made from ip ' + request.ip + ' data: ' + JSON.stringify(POST)); //displays in console purchase made from specific IP and quantities
...
function isNonNegInt(num_puppies, return_errors = false) {
  errors = []; // assume no errors at first
  if (num_puppies == '') num_puppies = 0; // handle blank inputs as if they are 0
  if (Number(num_puppies) != num_puppies) errors.push('<font color="red">Please enter number value!</font>'); // Check if string is a number value
  else if (num_puppies < 0) errors.push('<font color="red">Please put positive number!</font>'); // Check if it is non-negative
  else if (parseInt(num_puppies) != num_puppies) errors.push('<font color="red">Please put number value!</font>'); // Check that it is an integer
